...
class Astoundify_Updater_Extended_Location {
	...
	public function license_field( $fields ) {
		$fields[ 'wpjmel_settings' ][1][] = array(
			'name'			=> $this->slug,
			'type'          => 'license',
			'std'			=> '',
			'placeholder'	=> '',
			'label'			=> __( 'License Key', 'wp-job-manager-extended-location' ),
			'desc'			=> __( 'Enter the license key you received with your purchase receipt.', 'wp-job-manager-extended-location' ),
			'attributes'	=> array()
		);
	...
	public function output_license_field( $option, $attributes, $value, $placeholder ) {
		...
		?>
			...
			<?php if( $status !== false && $status == 'valid' ) { ?>
				...
				?>
				<a href="<?php echo esc_url( add_query_arg( $args, wp_nonce_url( admin_url( '/edit.php?post_type=job_listing&page=job-manager-settings' ), 'astoundify_license_deactivate' ) ) ); ?>" class="button-secondary"><?php _e( 'Deactivate', 'wp-job-manager-extended-location' ); ?></a>
			<?php  } ?>
			</p>
		<?php
	}
