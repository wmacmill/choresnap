...
class WC_Subscriptions_Cart {
	...
	public static function init() {
		...
		add_action( 'woocommerce_cart_loaded_from_session', __CLASS__ . '::get_cart_from_session' );
	...
	public static function set_subscription_prices_for_calculation( $price, $product ) {
		...
		if ( WC_Subscriptions_Product::is_subscription( $product ) ) {
			...
			WC()->cart->base_recurring_prices[ $product_id ] = $price;

			$sign_up_fee = WC_Subscriptions_Product::get_sign_up_fee( $product );

			WC()->cart->base_sign_up_fees[ $product_id ] = $sign_up_fee;
	...
	public static function calculate_subscription_totals( $total, $cart ) {
		...
		foreach ( $recurring_cart->get_cart() as $cart_item_key => $values ) {
			...
			if ( WC_Subscriptions_Product::is_subscription( $values['product_id'] ) ) {
				...
				$woocommerce->cart->recurring_cart_contents[ $values['product_id'] ]['recurring_line_tax_data']     = $values['line_tax_data'];
			}
		}
		...
		if ( isset( $recurring_cart->discount_cart_tax ) ) { // WC 2.3
			$woocommerce->cart->recurring_discount_cart_tax = $recurring_cart->discount_cart_tax;
		}

		if ( isset( $recurring_cart->discount_total ) ) { // WC < 2.3
	...
	public static function cart_coupon_discount_amount_html( $discount_html, $coupon ) {
		...
		if ( self::cart_contains_subscription() ) {
			...
			// Don't display discount when it's -$0.00 / month (probably because the coupon only provides free shipping)
			if ( '' !== $discount_html || $recurring_discount > 0 ) {
	...
	public static function get_recurring_discount_cart_tax() {
		...
		return $woocommerce->cart->recurring_discount_cart_tax;
	}
	...
	public static function get_recurring_taxes_total( $compound = true ) {
		...
		foreach ( self::get_recurring_taxes() as $tax_id => $tax_amount ) {
			...
			if ( ! $compound && WC_Tax::is_compound( $tax_id ) ) {
				continue;
			}
