...
<form action="<?php bp_forum_topic_action(); ?>" method="post" id="forum-topic-form" class="standard-form">
	<div class="item-list-tabs no-ajax" id="subnav" role="navigation">
		<ul>
			<?php if ( is_user_logged_in() ) : ?>
				...
				<li>
					<a href="<?php bp_forum_topic_new_reply_link(); ?>" class="new-reply-link"><?php _e( 'New Reply', 'appbuddy' ); ?></a>
				</li>
			...
			<?php if ( bp_forums_has_directory() ) : ?>
				...
				<li>
					<a href="<?php bp_forums_directory_permalink(); ?>"><?php _e( 'Forum Directory', 'appbuddy' ); ?></a>
				</li>
	...
	<div id="topic-meta">
		...
		<?php if ( bp_forum_topic_has_tags() ) : ?>
			...
			<div class="topic-tags">
				...
				<?php _e( 'Topic tags:', 'appbuddy' ); ?> <?php bp_forum_topic_tag_list(); ?>
			...
			<?php while ( bp_forum_topic_posts() ) : bp_the_forum_topic_post(); ?>
				...
				<li id="post-<?php bp_the_topic_post_id(); ?>" class="<?php bp_the_topic_post_css_class(); ?>">
					<div class="poster-meta">
						...
						<?php echo sprintf( __( '%1$s said %2$s:', 'appbuddy' ), bp_get_the_topic_post_poster_name(), bp_get_the_topic_post_time_since() ); ?>
					</div>
					...
					<div class="admin-links">
						...
						<a href="#post-<?php bp_the_topic_post_id(); ?>" title="<?php esc_attr_e( 'Permanent link to this post', 'appbuddy' ); ?>">#</a>
					</div>
				</li>
	...
	<?php else: ?>
		...
		<div id="message" class="info">
			<p><?php _e( 'There are no posts for this topic.', 'appbuddy' ); ?></p>
		</div>
	...
	<?php if ( ( is_user_logged_in() && 'public' == bp_get_group_status() ) || bp_group_is_member() ) : ?>
		...
		<?php if ( bp_get_the_topic_is_last_page() ) : ?>
			...
			<?php if ( bp_get_the_topic_is_topic_open() && !bp_group_is_user_banned() ) : ?>
				...
				<div id="post-topic-reply">
					...
					<?php if ( bp_groups_auto_join() && !bp_group_is_member() ) : ?>
						<p><?php _e( 'You will auto join this group when you reply to this topic.', 'appbuddy' ); ?></p>
					<?php endif; ?>

					<?php do_action( 'groups_forum_new_reply_before' ); ?>

					<h4><?php _e( 'Add a reply:', 'appbuddy' ); ?></h4>

					<textarea name="reply_text" id="reply_text"></textarea>

					<div class="submit">
						<input type="submit" name="submit_reply" id="submit" value="<?php esc_attr_e( 'Post Reply', 'appbuddy' ); ?>" />
					</div>
			...
			<?php elseif ( !bp_group_is_user_banned() ) : ?>
				...
				<div id="message" class="info">
					<p><?php _e( 'This topic is closed, replies are no longer accepted.', 'appbuddy' ); ?></p>
				</div>
