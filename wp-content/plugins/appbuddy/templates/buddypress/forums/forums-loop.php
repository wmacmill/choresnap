...
<?php if ( bp_has_forum_topics( bp_ajax_querystring( 'forums' ) ) ) : ?>
	...
	<table class="forum">
		<thead>
			<tr>
				<th id="th-title"><?php _e( 'Topic', 'appbuddy' ); ?></th>
				<th id="th-postcount"><?php _e( 'Posts', 'appbuddy' ); ?></th>
				<th id="th-freshness"><?php _e( 'Freshness', 'appbuddy' ); ?></th>
		...
		<tbody>
			...
			<tr class="<?php bp_the_topic_css_class(); ?>">
				<td class="td-title">
					<a class="topic-title" href="<?php bp_the_topic_permalink(); ?>" title="<?php esc_attr_e( 'Permanent link to this post', 'appbuddy' ); ?>">
					...
					<p class="topic-meta">
						<span class="topic-by"><?php /* translators: "started by [poster] in [forum]" */ printf( __( 'Started by %1$s', 'appbuddy' ), bp_get_the_topic_poster_avatar( 'height=20&width=20') . bp_get_the_topic_poster_name() ); ?></span>
						...
						<?php if ( !bp_is_group_forum() ) : ?>
							...
							<span class="topic-in">
								...
								<?php
									...
									printf( __( 'in %1$s', 'appbuddy' ), $topic_in );
								?>
...
<?php else: ?>
	...
	<div id="message" class="info">
		<p><?php _e( 'Sorry, there were no forum topics found.', 'appbuddy' ); ?></p>
	</div>
