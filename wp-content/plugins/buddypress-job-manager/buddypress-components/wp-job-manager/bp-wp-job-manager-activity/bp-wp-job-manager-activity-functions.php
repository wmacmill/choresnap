 ...
 */
function bp_job_listing_has_directory() {
	global $bp;
 ...
 */
function bp_job_listing_get_blogs( $args = '' ) {
	...
	// Filter and return
	return apply_filters( 'bp_job_listing_get_blogs', $blogs, $r );
}
 ...
 * @uses get_users()
 * @uses bp_job_listing_record_blog()
 */
function bp_job_listing_record_existing_blogs() {
	global $wpdb;
	...
	foreach ( (array) $blog_ids as $blog_id ) {
		...
		foreach ( (array) $users as $user ) {
			bp_job_listing_add_user_to_blog( $user->ID, false, $blog_id );
		}
	}
 ...
 */
function bp_job_listing_is_blog_recordable( $blog_id, $user_id = 0 ) {

	$recordable_globally = apply_filters( 'bp_job_listing_is_blog_recordable', true, $blog_id );

	if ( !empty( $user_id ) ) {
		$recordable_for_user = apply_filters( 'bp_job_listing_is_blog_recordable_for_user', $recordable_globally, $blog_id, $user_id );
	} else {
 ...
 * @since BuddyPress (1.7.0)
 *
 * @uses bp_job_listing_is_blog_recordable
 * @uses apply_filters()
 *
 * @param int $blog_id ID of the blog being checked.
 * @param int $user_id Optional. ID of the user for whom access is being checked.
 ...
 */
function bp_job_listing_is_blog_trackable( $blog_id, $user_id = 0 ) {

	$trackable_globally = apply_filters( 'bp_job_listing_is_blog_trackable', bp_job_listing_is_blog_recordable( $blog_id, $user_id ), $blog_id );

	if ( !empty( $user_id ) ) {
		$trackable_for_user = apply_filters( 'bp_job_listing_is_blog_trackable_for_user', $trackable_globally, $blog_id, $user_id );
	} else {
 ...
 */
function bp_job_listing_record_blog( $blog_id, $user_id, $no_activity = false ) {

	if ( empty( $user_id ) )
		$user_id = bp_loggedin_user_id();

	// If blog is not recordable, do not record the activity.
	if ( !bp_job_listing_is_blog_recordable( $blog_id, $user_id ) )
		return false;
	...
	bp_job_listing_update_blogmeta( $recorded_blog->blog_id, 'url', $url );
	bp_job_listing_update_blogmeta( $recorded_blog->blog_id, 'name', $name );
	bp_job_listing_update_blogmeta( $recorded_blog->blog_id, 'description', $description );
	bp_job_listing_update_blogmeta( $recorded_blog->blog_id, 'last_activity', bp_core_current_time() );
	bp_job_listing_update_blogmeta( $recorded_blog->blog_id, 'close_comments_for_old_posts', $close_old_posts );
	bp_job_listing_update_blogmeta( $recorded_blog->blog_id, 'close_comments_days_old', $close_days_old );
	bp_job_listing_update_blogmeta( $recorded_blog->blog_id, 'thread_comments_depth', $thread_depth );

	$is_private = !empty( $_POST['blog_public'] ) && (int) $_POST['blog_public'] ? false : true;
	$is_private = !apply_filters( 'bp_is_new_job_public', !$is_private );

	// Only record this activity if the blog is public
	if ( !$is_private && !$no_activity && bp_job_listing_is_blog_trackable( $blog_id, $user_id ) ) {

		// Record this in activity streams
		bp_job_listing_record_activity( array(
			'user_id'      => $recorded_blog->user_id,
			'primary_link' => apply_filters( 'bp_job_listing_activity_created_blog_primary_link', $url, $recorded_blog->blog_id ),
			'type'         => 'new_job',
			'item_id'      => $recorded_blog->blog_id
		) );
	}

	do_action_ref_array( 'bp_job_listing_new_job', array( &$recorded_blog, $is_private, $is_recorded ) );
}
add_action( 'wpmu_new_job', 'bp_job_listing_record_blog', 10, 2 );
 ...
 */
function bp_job_listing_update_option_blogname( $oldvalue, $newvalue ) {
	global $wpdb;

	bp_job_listing_update_blogmeta( $wpdb->blogid, 'name', $newvalue );
}
add_action( 'update_option_blogname', 'bp_job_listing_update_option_blogname', 10, 2 );
 ...
 * @param string $newvalue Value to change meta to.
 */
function bp_job_listing_update_option_blogdescription( $oldvalue, $newvalue ) {
	global $wpdb;

	bp_job_listing_update_blogmeta( $wpdb->blogid, 'description', $newvalue );
}
add_action( 'update_option_blogdescription', 'bp_job_listing_update_option_blogdescription', 10, 2 );
 ...
 * @param string $newvalue Value to change meta to.
 */
function bp_job_listing_update_option_close_comments_for_old_posts( $oldvalue, $newvalue ) {
	global $wpdb;

	bp_job_listing_update_blogmeta( $wpdb->blogid, 'close_comments_for_old_posts', $newvalue );
}
add_action( 'update_option_close_comments_for_old_posts', 'bp_job_listing_update_option_close_comments_for_old_posts', 10, 2 );
 ...
 * @param string $newvalue Value to change meta to.
 */
function bp_job_listing_update_option_close_comments_days_old( $oldvalue, $newvalue ) {
	global $wpdb;

	bp_job_listing_update_blogmeta( $wpdb->blogid, 'close_comments_days_old', $newvalue );
}
add_action( 'update_option_close_comments_days_old', 'bp_job_listing_update_option_close_comments_days_old', 10, 2 );
 ...
 * @param string $newvalue Value to change meta to.
 */
function bp_job_listing_update_option_thread_comments( $oldvalue, $newvalue ) {
	global $wpdb;
	...
	bp_job_listing_update_blogmeta( $wpdb->blogid, 'thread_comments_depth', $thread_depth );
}
add_action( 'update_option_thread_comments', 'bp_job_listing_update_option_thread_comments', 10, 2 );
 ...
 * @param string $newvalue Value to change meta to.
 */
function bp_job_listing_update_option_thread_comments_depth( $oldvalue, $newvalue ) {
	global $wpdb;

	$comments_enabled = get_option( 'thread_comments' );

	if (  $comments_enabled ) {
		bp_job_listing_update_blogmeta( $wpdb->blogid, 'thread_comments_depth', $newvalue );
	}
}
add_action( 'update_option_thread_comments_depth', 'bp_job_listing_update_option_thread_comments_depth', 10, 2 );
 ...
 */
function bp_job_listing_catch_transition_post_status( $new_status, $old_status, $post ) {

	// This is an edit
	if ( $new_status === $old_status ) {
		if ( $new_status == 'publish' ) {
			bp_job_listing_update_post( $post );
			return;
		}
	}
...
add_action( 'transition_post_status', 'bp_job_listing_catch_transition_post_status', 10, 3 );
 ...
 * @return bool|null Returns false on failure.
 */
function bp_job_listing_record_post( $post_id, $post, $user_id = 0 ) {
	global $bp, $wpdb;
	...
	if ( ! bp_job_listing_is_blog_trackable( $blog_id, $user_id ) )
		return false;
	...
	if ( (int) $blog_id == $tags_blog_id && apply_filters( 'bp_job_listing_block_sitewide_tags_activity', true ) )
		return false;

	// Don't record this if it's not a job_listing
	if ( !in_array( $post->post_type, apply_filters( 'bp_job_listing_record_post_post_types', array( 'job_listing' ) ) ) )
		return false;
	...
	if ( 'publish' == $post->post_status && empty( $post->post_password ) ) {
		if ( $is_blog_public || !is_multisite() ) {
			...
			if ( is_multisite() )
				$activity_action  = sprintf( __( '%1$s posted a new job, %2$s, on the site %3$s', 'buddypress' ), bp_core_get_userlink( (int) $post->post_author ), '<a href="' . $post_permalink . '">' . $post->post_title . '</a>', '<a href="' . get_blog_option( $blog_id, 'home' ) . '">' . get_blog_option( $blog_id, 'blogname' ) . '</a>' );
			else
				$activity_action  = sprintf( __( '%1$s posted a new job, %2$s', 'buddypress' ), bp_core_get_userlink( (int) $post->post_author ), '<a href="' . $post_permalink . '">' . $post->post_title . '</a>' );
			...
			$activity_id = bp_job_listing_record_activity( array(
				'user_id'           => (int) $post->post_author,
				'content'           => apply_filters( 'bp_job_listing_activity_new_post_content',      $activity_content, $post, $post_permalink ),
				'primary_link'      => apply_filters( 'bp_job_listing_activity_new_post_primary_link', $post_permalink,   $post_id               ),
				'type'              => 'new_job_post',
				'item_id'           => $blog_id,
		...
		bp_job_listing_update_blogmeta( $blog_id, 'last_activity', bp_core_current_time() );
	} else {
		bp_job_listing_remove_post( $post_id, $blog_id, $user_id );
	}

	do_action( 'bp_job_listing_new_job_post', $post_id, $post, $user_id );
}
 ...
 * @since BuddyPress (2.0.0)
 *
 * @see bp_job_listing_catch_transition_post_status()
 *
 * @param WP_Post $post
 */
function bp_job_listing_update_post( $post ) {
	if ( ! bp_is_active( 'activity' ) ) {
		return;
	}
	...
	if ( ! empty( $post->post_content ) ) {
		...
		$activity->content = apply_filters( 'bp_job_listing_record_activity_content', bp_create_excerpt( $post_content ), $post_content, (array) $activity );
	}
	...
	if ( $post->post_title !== $existing_title ) {
		...
		add_filter( 'comments_clauses', 'bp_job_listing_comments_clauses_select_by_id' );
		$comments = get_comments( array( 'post_id' => $post->ID ) );
		remove_filter( 'comments_clauses', 'bp_job_listing_comments_clauses_select_by_id' );
		...
		if ( ! empty( $comments ) ) {
			...
			// query for old-style "new_job_comment" activity items
			$args['filter'] = array(
				'object'       => buddypress()->blogs->id,
				'action'       => 'new_job_comment',
			...
			$args['meta_query'] = array( array(
				'key'     => 'bp_job_listing_post_comment_id',
				'value'   => $comment_ids,
				'compare' => 'IN',
			) );
 ...
 */
function bp_job_listing_record_comment( $comment_id, $is_approved = true ) {
	...
	if ( !in_array( $recorded_comment->post->post_type, apply_filters( 'bp_job_listing_record_comment_post_types', array( 'job_listing' ) ) ) )
		return false;
	...
	if ( $is_blog_public ) {
		...
		$args['content']       = apply_filters_ref_array( 'bp_job_listing_activity_new_comment_content', array( $recorded_comment->comment_content, &$recorded_comment, $comment_link ) );
		$args['primary_link']  = apply_filters_ref_array( 'bp_job_listing_activity_new_comment_primary_link', array( $comment_link,     &$recorded_comment ) );
		...
		// record comment as BP activity comment under the parent 'new_job_post'
		// activity item
		} else {
			...
			// find the parent 'new_job_post' activity entry
			...
			if ( ! empty( $parent_activity_id ) ) {
				...
				// so set to 'new_job_post' activity ID
			...
			if ( ! empty( $args ) ) {
				...
				if ( empty( $args['id'] ) ) {
					...
					bp_activity_update_meta( $comment_activity_id, 'bp_job_listing_post_comment_id', $comment_id );
...
add_action( 'comment_post', 'bp_job_listing_record_comment', 10, 2 );
add_action( 'edit_comment', 'bp_job_listing_record_comment', 10    );
 ...
 * default, though as of 2.1.0 you can filter 'bp_job_listing_get_allowed_roles' to
 ...
 */
function bp_job_listing_add_user_to_blog( $user_id, $role = false, $blog_id = 0 ) {
	global $wpdb;
	...
	if ( empty( $role ) || ! in_array( $role, bp_job_listing_get_allowed_roles() ) ) {
		return false;
	}

	// Record the blog activity for this user being added to this blog
	bp_job_listing_record_blog( $blog_id, $user_id, true );
}
add_action( 'add_user_to_blog', 'bp_job_listing_add_user_to_blog', 10, 3 );
add_action( 'profile_update',   'bp_job_listing_add_user_to_blog'        );
add_action( 'user_register',    'bp_job_listing_add_user_to_blog'        );
 ...
 * @return string
 */
function bp_job_listing_get_allowed_roles() {
	return apply_filters( 'bp_job_listing_get_allowed_roles', array( 'contributor', 'author', 'editor', 'administrator' ) );
}
 ...
 */
function bp_job_listing_remove_user_from_blog( $user_id, $blog_id = 0 ) {
	global $wpdb;
	...
	bp_job_listing_remove_blog_for_user( $user_id, $blog_id );
}
add_action( 'remove_user_from_blog', 'bp_job_listing_remove_user_from_blog', 10, 2 );
 ...
 * @access private
 */
function bp_job_listing_maybe_add_user_to_blog() {
	if ( ! is_multisite() )
		return;
...
add_action( 'init', 'bp_job_listing_maybe_add_user_to_blog', 1 );
 ...
 */
function bp_job_listing_remove_blog( $blog_id ) {
	global $bp;

	$blog_id = (int) $blog_id;
	do_action( 'bp_job_listing_before_remove_blog', $blog_id );

	BP_Blogs_Blog::delete_blog_for_all( $blog_id );

	// Delete activity stream item
	bp_job_listing_delete_activity( array( 'item_id' => $blog_id, 'component' => $bp->blogs->id, 'type' => 'new_job' ) );

	do_action( 'bp_job_listing_remove_blog', $blog_id );
}
add_action( 'delete_blog', 'bp_job_listing_remove_blog' );
 ...
 * @param int $blog_id ID of the blog being removed.
 */
function bp_job_listing_remove_blog_for_user( $user_id, $blog_id ) {
	global $bp;

	$blog_id = (int) $blog_id;
	$user_id = (int) $user_id;

	do_action( 'bp_job_listing_before_remove_blog_for_user', $blog_id, $user_id );
	...
	do_action( 'bp_job_listing_remove_blog_for_user', $blog_id, $user_id );
}
add_action( 'remove_user_from_blog', 'bp_job_listing_remove_blog_for_user', 10, 2 );
 ...
 */
function bp_job_listing_remove_post( $post_id, $blog_id = 0, $user_id = 0 ) {
	global $wpdb, $bp;
	...
	do_action( 'bp_job_listing_before_remove_post', $blog_id, $post_id, $user_id );

	// Delete activity stream item
	bp_job_listing_delete_activity( array( 'item_id' => $blog_id, 'secondary_item_id' => $post_id, 'component' => $bp->blogs->id, 'type' => 'new_job_post' ) );

	do_action( 'bp_job_listing_remove_post', $blog_id, $post_id, $user_id );
}
add_action( 'delete_post', 'bp_job_listing_remove_post' );
 ...
 */
function bp_job_listing_remove_comment( $comment_id ) {
	global $wpdb;
	...
	// remove the associated activity item
		...
		if ( ! empty( $activity_id ) && bp_is_active( 'activity' ) ) {
			...
			if ( ! empty( $activity['activities'] ) ) {
				...
				bp_job_listing_remove_associated_blog_comments( $activity_ids );
	...
	do_action( 'bp_job_listing_remove_comment', $wpdb->blogid, $comment_id, bp_loggedin_user_id() );
}
add_action( 'delete_comment', 'bp_job_listing_remove_comment' );
 ...
 * @since BuddyPress (2.0.0)
 *
 * @see bp_job_listing_remove_comment()
 * @see bp_job_listing_sync_delete_from_activity_comment()
 *
 ...
 */
function bp_job_listing_remove_associated_blog_comments( $activity_ids = array(), $force_delete = true ) {
	// query args
	$query_args = array(
		'meta_query' => array(
			array(
 ...
 */
function bp_job_listing_transition_activity_status( $new_status, $old_status, $comment ) {
	global $bp;
	...
	// This clause was moved in from bp_job_listing_remove_comment() in BuddyPress 1.6. It handles delete/hold.
	if ( in_array( $new_status, array( 'delete', 'hold' ) ) ) {
		return bp_job_listing_remove_comment( $comment->comment_ID );
	...
	// Get the activity
	if ( bp_disable_blogforum_comments() ) {
		$activity_id = bp_activity_get_activity_id( array( 'component' => $bp->blogs->id, 'item_id' => get_current_blog_id(), 'secondary_item_id' => $comment->comment_ID, 'type' => 'new_job_comment', ) );
	} else {
	...
	if ( empty( $activity_id ) ) {
		...
		if ( 'approved' == $new_status ) {
			return bp_job_listing_record_comment( $comment->comment_ID, true );
		}
	...
	// Add "new_job_comment" to the whitelisted activity types, so that the activity's Akismet history is generated
	$comment_akismet_history = create_function( '$t', '$t[] = "new_job_comment"; return $t;' );
	add_filter( 'bp_akismet_get_activity_types', $comment_akismet_history );

	// Save the updated activity
	$activity->save();

	// Remove the "new_job_comment" activity type whitelist so we don't break anything
	remove_filter( 'bp_akismet_get_activity_types', $comment_akismet_history );
}
add_action( 'transition_comment_status', 'bp_job_listing_transition_activity_status', 10, 3 );
 ...
 */
function bp_job_listing_total_blogs() {
 ...
 */
function bp_job_listing_total_blogs_for_user( $user_id = 0 ) {
	if ( empty( $user_id ) ) {
 ...
 */
function bp_job_listing_remove_data_for_blog( $blog_id ) {
	global $bp;

	do_action( 'bp_job_listing_before_remove_data_for_blog', $blog_id );

	// If this is regular blog, delete all data for that blog.
	BP_Blogs_Blog::delete_blog_for_all( $blog_id );

	// Delete activity stream item
	bp_job_listing_delete_activity( array( 'item_id' => $blog_id, 'component' => $bp->blogs->id, 'type' => false ) );

	do_action( 'bp_job_listing_remove_data_for_blog', $blog_id );
}
add_action( 'delete_blog', 'bp_job_listing_remove_data_for_blog', 1 );
 ...
 */
function bp_job_listing_get_blogs_for_user( $user_id, $show_hidden = false ) {
 ...
 */
function bp_job_listing_get_all_blogs( $limit = null, $page = null ) {
 ...
 */
function bp_job_listing_get_random_blogs( $limit = null, $page = null ) {
 ...
 */
function bp_job_listing_is_blog_hidden( $blog_id ) {
 ...
 */
function bp_job_listing_delete_blogmeta( $blog_id, $meta_key = false, $meta_value = false, $delete_all = false ) {
	global $wpdb, $bp;
 ...
 */
function bp_job_listing_get_blogmeta( $blog_id, $meta_key = '', $single = true ) {
	add_filter( 'query', 'bp_filter_metaid_column_name' );
 ...
 */
function bp_job_listing_update_blogmeta( $blog_id, $meta_key, $meta_value, $prev_value = '' ) {
	add_filter( 'query', 'bp_filter_metaid_column_name' );
 ...
 */
function bp_job_listing_add_blogmeta( $blog_id, $meta_key, $meta_value, $unique = false ) {
	add_filter( 'query', 'bp_filter_metaid_column_name' );
 ...
 * @return bool|null Returns false on failure.
 */
function bp_job_listing_remove_data( $user_id ) {
	if ( !is_multisite() )
		return false;

	do_action( 'bp_job_listing_before_remove_data', $user_id );

	// If this is regular blog, delete all data for that blog.
	BP_Blogs_Blog::delete_blogs_for_user( $user_id );

	do_action( 'bp_job_listing_remove_data', $user_id );
}
add_action( 'wpmu_delete_user',  'bp_job_listing_remove_data' );
add_action( 'delete_user',       'bp_job_listing_remove_data' );
add_action( 'bp_make_spam_user', 'bp_job_listing_remove_data' );


//** Hooks **/
// For Future ref only ** DO NOT USE **/
// Thanks
/**
add_filter( 'bp_job_listing_activity_new_post_action', 'im_activity_content', 2, 10 );
...
function bbg_record_my_custom_post_type_posts( $post_types ) {
      $post_types = array ('job_listing', 'resume', 'job_application', 'post'); 
      return $post_types;
}
add_filter( 'bp_job_listing_record_post_post_types', 'bbg_record_my_custom_post_type_posts' );
  
function bbg_record_my_custom_post_type_comments( $post_types ) {
      $post_types = array ('job_listing', 'resume', 'job_application', 'post'); 
      return $post_types;
}
add_filter( 'bp_job_listing_record_comment_post_types', 'bbg_record_my_custom_post_type_comments' );
**/