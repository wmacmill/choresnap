 ...
 */
function bp_job_listing_register_activity_actions() {
	global $bp;
	...
	// if ( is_multisite() ) {
		// bp_activity_set_action(
			...
			// 'bp_job_listing_format_activity_action_new_job',
	...
	bp_activity_set_action(
		...
		'bp_job_listing_format_activity_action_new_job_post',
		__( 'Jobs', 'buddypress' ),
	...
	// bp_activity_set_action(
		...
		// 'bp_job_listing_format_activity_action_new_job_comment',
		// __( 'Job Comments', 'buddypress' ),
		// array( 'activity', 'member' )
	// );

	do_action( 'bp_job_listing_register_activity_actions' );
}
add_action( 'bp_register_activity_actions', 'bp_job_listing_register_activity_actions' );

/**
 * Format 'new_job' activity actions.
 *
 ...
 */
function bp_job_listing_format_activity_action_new_job( $action, $activity ) {
	$blog_url  = bp_job_listing_get_blogmeta( $activity->item_id, 'url' );
	$blog_name = bp_job_listing_get_blogmeta( $activity->item_id, 'name' );
	...
	if ( has_filter( 'bp_job_listing_activity_created_blog_action' ) ) {
		...
		if ( isset( $recorded_blog ) ) {
			$action = apply_filters( 'bp_job_listing_activity_created_blog_action', $action, $recorded_blog, $blog_name, bp_job_listing_get_blogmeta( $activity->item_id, 'description' ) );
		}
	}

	return apply_filters( 'bp_job_listing_format_activity_action_new_job', $action, $activity );
}
 ...
 * @param obj $activity Activity data object.
 */
function bp_job_listing_format_activity_action_new_job_post( $action, $activity ) {
	$blog_url  = bp_job_listing_get_blogmeta( $activity->item_id, 'url' );
	$blog_name = bp_job_listing_get_blogmeta( $activity->item_id, 'name' );
	...
	if ( empty( $blog_url ) || empty( $blog_name ) ) {
		...
		bp_job_listing_update_blogmeta( $activity->item_id, 'url', $blog_url );
		bp_job_listing_update_blogmeta( $activity->item_id, 'name', $blog_name );
	}
	...
	if ( is_multisite() ) {
		$action  = sprintf( __( '%1$s posted a new job, %2$s, on the site %3$s', 'buddypress' ), $user_link, $post_link, '<a href="' . esc_url( $blog_url ) . '">' . esc_html( $blog_name ) . '</a>' );
	} else {
		$action  = sprintf( __( '%1$s posted a new job, %2$s', 'buddypress' ), $user_link, $post_link );
	}

	// Legacy filter - requires the post object
	if ( has_filter( 'bp_job_listing_activity_new_post_action' ) ) {
		switch_to_blog( $activity->item_id );
		$post = get_post( $activity->secondary_item_id );
		restore_current_blog();

		if ( ! empty( $post ) && ! is_wp_error( $post ) ) {
			$action = apply_filters( 'bp_job_listing_activity_new_post_action', $action, $post, $post_url );
		}
	}

	return apply_filters( 'bp_job_listing_format_activity_action_new_job_post', $action, $activity );
}

/**
 * Format 'new_job_comment' activity actions.
 *
 * @since BuddyPress (2.0.0)
 *
 * @param string $action Static activity action.
 * @param obj $activity Activity data object.
 */
function bp_job_listing_format_activity_action_new_job_comment( $action, $activity ) {
	$blog_url  = bp_job_listing_get_blogmeta( $activity->item_id, 'url' );
	$blog_name = bp_job_listing_get_blogmeta( $activity->item_id, 'name' );
	...
	if ( has_filter( 'bp_job_listing_activity_new_comment_action' ) ) {
		switch_to_blog( $activity->item_id );
		$comment = get_comment( $activity->secondary_item_id );
		restore_current_blog();

		if ( ! empty( $comment ) && ! is_wp_error( $comment ) ) {
			$action = apply_filters( 'bp_job_listing_activity_new_comment_action', $action, $comment, $post_url . '#' . $activity->secondary_item_id );
		}
	}

	return apply_filters( 'bp_job_listing_format_activity_action_new_job_comment', $action, $activity );
}
 ...
 * @return array
 */
function bp_job_listing_prefetch_activity_object_data( $activities ) {
	if ( empty( $activities ) ) {
		return $activities;
	}
...
add_filter( 'bp_activity_prefetch_object_data', 'bp_job_listing_prefetch_activity_object_data' );
 ...
 */
function bp_job_listing_record_activity( $args = '' ) {
	global $bp;
	...
	if ( ! empty( $r['action'] ) ) {
		$r['action'] = apply_filters( 'bp_job_listing_record_activity_action', $r['action'] );
	}

	if ( ! empty( $r['content'] ) ) {
		$r['content'] = apply_filters( 'bp_job_listing_record_activity_content', bp_create_excerpt( $r['content'] ), $r['content'], $r );
	}
 ...
 */
function bp_job_listing_delete_activity( $args = '' ) {
 ...
 * @return bool
 */
function bp_job_listing_comments_open( $activity ) {
	$open = true;

	$blog_id = $activity->item_id;

	// see if we've mirrored the close comments option before
	$days_old = bp_job_listing_get_blogmeta( $blog_id, 'close_comments_days_old' );
	...
	if ( '' === $days_old ) {
		...
		bp_job_listing_update_blogmeta( $blog_id, 'close_comments_for_old_posts', get_option( 'close_comments_for_old_posts' ) );
		bp_job_listing_update_blogmeta( $blog_id, 'close_comments_days_old',      get_option( 'close_comments_days_old' ) );
		bp_job_listing_update_blogmeta( $blog_id, 'thread_comments_depth',        $thread_depth );
	...
	// basically a copy of _close_comments_for_old_post()
		...
		if ( ! bp_job_listing_get_blogmeta( $blog_id, 'close_comments_for_old_posts' ) ) {
			return $open;
		}
 ...
 *
 * For blog post -> activity comment, see {@link bp_job_listing_record_comment()}.
 *
 * @since BuddyPress (2.0.0)
 *
 ...
 */
function bp_job_listing_sync_add_from_activity_comment( $comment_id, $params, $parent_activity ) {
	// if parent activity isn't a blog post, stop now!
	if ( $parent_activity->type != 'new_job_post' ) {
		return;
	}
	...
	if ( ! empty( $params['parent_id'] ) ) {
		$comment_parent = bp_activity_get_meta( $params['parent_id'], 'bp_job_listing_post_comment_id' );
	} else {
		$comment_parent = 0;
	}
	...
	remove_action( 'comment_post', 'bp_job_listing_record_comment', 10, 2 );
	...
	bp_activity_update_meta( $comment_id, 'bp_job_listing_post_comment_id', $post_comment_id );

	// resave activity comment with WP comment permalink
	//
	// in bp_job_listing_activity_comment_permalink(), we change activity comment
	...
	do_action( 'bp_job_listing_sync_add_from_activity_comment', $comment_id, $args, $parent_activity, $user );
}
add_action( 'bp_activity_comment_posted', 'bp_job_listing_sync_add_from_activity_comment', 10, 3 );
 ...
 */
function bp_job_listing_sync_delete_from_activity_comment( $retval, $parent_activity_id, $activity_id ) {
	// check if parent activity is a blog post
	$parent_activity = new BP_Activity_Activity( $parent_activity_id );
	if ( 'new_job_post' != $parent_activity->type ) {
		return $retval;
	}
	...
	bp_job_listing_remove_associated_blog_comments( $activity_ids, current_user_can( 'moderate_comments' ) );
...
add_filter( 'bp_activity_delete_comment_pre', 'bp_job_listing_sync_delete_from_activity_comment', 10, 3 );
 ...
 */
function bp_job_listing_sync_activity_edit_to_post_comment( BP_Activity_Activity $activity ) {
	...
	// prevent recursion
	remove_action( 'bp_activity_before_save', 'bp_job_listing_sync_activity_edit_to_post_comment', 20 );

	// Try to see if a corresponding blog comment exists
	$post_comment_id = bp_activity_get_meta( $activity->id, 'bp_job_listing_post_comment_id' );
...
add_action( 'bp_activity_before_save', 'bp_job_listing_sync_activity_edit_to_post_comment', 20 );
 ...
 */
function bp_job_listing_remove_activity_meta_for_trashed_comments( $post_id = 0, $comments = array() ) {
	if ( ! empty( $comments ) ) {
...
add_action( 'trashed_post_comments', 'bp_job_listing_remove_activity_meta_for_trashed_comments', 10, 2 );

/**
 * Filter 'new_job_comment' bp_has_activities() loop to include new- and old-style blog activity comment items.
 *
 * In BuddyPress 2.0, the schema for storing activity items related to blog
 * posts changed. Instead creating new top-level 'new_job_comment' activity
 * items, blog comments are recorded in the activity stream as comments on the
 * 'new_job_post' activity items corresponding to the parent post. This filter
 * ensures that the 'new_job_comment' filter in bp_has_activities() (which
 ...
 * @return array $args
 */
function bp_job_listing_new_job_comment_query_backpat( $args ) {
	// Bail if this is not a 'new_job_comment' query
	if ( 'new_job_comment' !== $args['action'] ) {
		return $args;
	}
	...
	add_filter( 'bp_activity_paged_activities_sql', '_bp_job_listing_new_job_comment_query_backpat_filter' );
	add_filter( 'bp_activity_total_activities_sql', '_bp_job_listing_new_job_comment_query_backpat_filter' );

	// Return the original arguments
	return $args;
}
add_filter( 'bp_after_has_activities_parse_args', 'bp_job_listing_new_job_comment_query_backpat' );

/**
 * Filter activity SQL to include new- and old-style 'new_job_comment' activity items.
 *
 * @since BuddyPress (2.1.0)
 *
 * @access private
 * @see bp_job_listing_new_job_comment_query_backpat()
 *
 * @param string $query SQL query as assembled in BP_Activity_Activity::get().
 * @return string $query Modified SQL query.
 */
function _bp_job_listing_new_job_comment_query_backpat_filter( $query ) {
	$bp = buddypress();

	// The query passed to the filter is for old-style 'new_job_comment'
	// items. We include new-style 'activity_comment' items by running a
	// subquery inside of a large OR clause.
	$activity_comment_subquery = "SELECT a.id FROM {$bp->activity->table_name} a INNER JOIN {$bp->activity->table_name_meta} am ON (a.id = am.activity_id) WHERE am.meta_key = 'bp_job_listing_post_comment_id' AND a.type = 'activity_comment'";
	...
	remove_filter( current_filter(), '_bp_job_listing_new_job_comment_query_backpat_filter' );
 ...
 * @since BuddyPress (2.0.0)
 *
 * @see bp_job_listing_disable_activity_commenting()
 * @see bp_job_listing_setup_comment_loop_globals_on_ajax()
 *
 * @param object $activity The BP_Activity_Activity object
 */
function bp_job_listing_setup_activity_loop_globals( $activity ) {
	if ( ! is_object( $activity ) ) {
		return;
	}

	// parent not a blog post? stop now!
	if ( 'new_job_post' !== $activity->type ) {
		return;
	}
	...
	$allow_comments = bp_job_listing_comments_open( $activity );
	$thread_depth   = bp_job_listing_get_blogmeta( $activity->item_id, 'thread_comments_depth' );
	...
	// @see bp_job_listing_disable_activity_replies()
	//
 ...
 * @since BuddyPress (2.0.0)
 *
 * @see bp_job_listing_setup_activity_loop_globals()
 */
function bp_job_listing_setup_comment_loop_globals_on_ajax() {
	// not AJAX? stop now!
	...
	// setup the globals
	bp_job_listing_setup_activity_loop_globals( $parent_activity );
}
add_action( 'bp_before_activity_comment', 'bp_job_listing_setup_comment_loop_globals_on_ajax' );
 ...
 *  - the activity entry is a 'new_job_comment' type
 *
 * @since BuddyPress (2.0.0)
 *
 ...
 * @return bool
 */
function bp_job_listing_disable_activity_commenting( $retval ) {
	...
	switch ( bp_get_activity_action_name() ) {
		...
		// we still have to disable activity commenting for 'new_job_comment' items
		// commenting should only be done on the parent 'new_job_post' item
		case 'new_job_comment' :
			$retval = false;
		...
		case 'new_job_post' :
			...
			bp_job_listing_setup_activity_loop_globals( $activities_template->activity );
...
add_filter( 'bp_activity_can_comment', 'bp_job_listing_disable_activity_commenting' );
 ...
 *
 * This check uses a locally-cached value set in {@link bp_job_listing_disable_activity_commenting()}
 * via {@link bp_job_listing_setup_activity_loop_globals()}.
 *
 * @since BuddyPress (2.0.0)
 *
 ...
 * @return bool
 */
function bp_job_listing_can_comment_reply( $retval, $comment ) {
	if ( is_array( $comment ) ) {
	...
	if ( isset( buddypress()->blogs->allow_comments[$comment->item_id] ) ){
		...
		// comments under the parent 'new_job_post' activity entry
...
add_filter( 'bp_activity_can_comment_reply', 'bp_job_listing_can_comment_reply', 10, 2 );
 ...
 * activity item is a 'new_job_post' entry.
 *
 * @since BuddyPress (2.0.0)
 *
 ...
 * @return string
 */
function bp_job_listing_activity_comment_permalink( $retval = '' ) {
	global $activities_template;
...
add_filter( 'bp_get_activity_comment_permalink', 'bp_job_listing_activity_comment_permalink' );
 ...
 * @return string
 */
function bp_job_listing_activity_comment_single_permalink( $retval, $activity ) {
	if ( 'activity_comment' !== $activity->type ) {
		return $retval;
	}

	$blog_comment_id = bp_activity_get_meta( $activity->id, 'bp_job_listing_post_comment_id' );
...
add_filter( 'bp_activity_get_permalink', 'bp_job_listing_activity_comment_single_permalink', 10, 2 );
 ...
 * @return string
 */
function bp_job_listing_activity_comment_single_action( $retval, $activity ) {
	if ( 'activity_comment' !== $activity->type ) {
		return $retval;
	}
	...
	if ( ! empty( $blog_comment_id ) ) {
		...
		// fake a 'new_job_comment' activity object
		$object = $activity;
		...
		// now format the activity action using the 'new_job_comment' action callback
		$retval = bp_job_listing_format_activity_action_new_job_comment( '', $object );
	}

	return $retval;
}
add_filter( 'bp_get_activity_action_pre_meta', 'bp_job_listing_activity_comment_single_action', 10, 2 );