...
<ul class="actions">
	...
	<?php if ( $email = get_job_application_email( $application->ID ) ) : ?>
		<li class="email"><a href="mailto:<?php echo esc_attr( $email ); ?>?subject=<?php echo esc_attr( sprintf( __( 'Your estimate for %s', 'wp-job-manager-applications' ), get_the_title( $job_id ) ) ); ?>" title="<?php _e( 'Email', 'wp-job-manager-applications' ); ?>" class="job-application-contact"><?php _e( 'Email', 'wp-job-manager-applications' ); ?></a></li>
	<?php endif; ?>
	...
	<?php if ( ( $resume_id = get_job_application_resume_id( $application->ID ) ) && 'publish' === get_post_status( $resume_id ) && function_exists( 'get_resume_share_link' ) && ( $share_link = get_resume_share_link( $resume_id ) ) ) : ?>
		<li class="resume"><a class="ajaxify" href="<?php echo esc_attr( $share_link ); ?>" target="_blank" class="job-application-resume"><?php echo esc_html( $resume_id ); ?></a></li>
	<?php endif; ?>
